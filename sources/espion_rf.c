...
/* Structure pour les messages envoyés par la carte */
struct TX_PACKET {
   uint8_t length; /* (1 octet, = N + 2) */
   uint8_t addr_low; /* (un octet) */
   uint8_t addr_high; /* (un octet) */
   uint8_t data[TAILLE_MSG]; /* (N octets) */
};

/* Structure pour les messages envoyés par le camion */
struct RX_PACKET {
   uint8_t length; /* Taille du paquet - 1 : toujours 15 */
   uint8_t bcast; /* Always 0xFF */
   uint8_t addr_low; /* addr_low */
   uint8_t addr_high;
...
#endif
	serial_write(0, "data: ", 6);
	while (cnt < ret) {
		len = snprintf(buff, BUFF_LEN, "0x%02x ", data[cnt++]);
		serial_write(0, buff, len);
	}
	serial_write(0, "\n", 1);

	/* Si l'adresse est celle de broadcast, alors c'est le message d'un camion */
	if (data[1] == 0xFF) {
		struct RX_PACKET* msg = (struct RX_PACKET*)data;
		len = snprintf(buff, BUFF_LEN, "Message d'un camion => @camion :%s\r\n", msg->addr_low);
		serial_write(0, buff, len);
	 } 
	/*Sinon, c'est un packet de la carte */	
	else {
		struct TX_PACKET* msg = (struct TX_PACKET*) data;
		len = snprintf(buff, BUFF_LEN, "Message vers le camion => @camion :%d, msg : %s\r\n",msg->addr_low, (char*)msg->data);
		serial_write(0, buff, len);
